...
class Emprestimo_CRUD:
    ...
    async def getAllEmprestimosFromProfessores():
        try:
            with Session(engine) as session:
                query = select('*').select_from(text('getallemprestimosfromprofessores'))
                result = session.execute(query).all()
                res = []
                for row in result:
                    emprestimo = Emprestimo(
    ...
    async def createEmprestimo(new_emprestimo: list[Emprestimo]):
        try:
            with Session(engine) as session:
                
                emprestimos = [emprestimo_data.__dict__ for emprestimo_data in new_emprestimo]
                
                for emprestimo in emprestimos:
                    emprestimo.pop("_sa_instance_state", None)
    ...
    async def updateEmprestimo(Id: int, new_value: Emprestimo):
        try:
            with Session(engine) as session:
    ...
    async def deleteEmprestimo(Id: int):
        try:
            with Session(engine) as session:
                ...
                result = session.execute(delete(Emprestimo).
                                        where(Emprestimo.id == Id))
                session.commit()
